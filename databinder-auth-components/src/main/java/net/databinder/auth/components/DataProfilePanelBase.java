...
public abstract class DataProfilePanelBase<T extends DataUser> extends Panel {
  ...
  public DataProfilePanelBase(final String id, final ReturnPage returnPage) {
    super(id);
  ...
  protected boolean existing() {
    final BindingModel model = (BindingModel<T>)((IChainingModel<T>)form.getModel()).getChainedModel();
  ...
  protected void afterSubmit() {
    getAuthSession().signIn(getUser(), rememberMe.getModelObject());
  ...
  public static boolean isAvailable(final String username) {
    final AuthApplication authSettings = (AuthApplication)Application.get();

    final DataUser found = authSettings.getUser(username),
  ...
  public static class UsernameValidator extends StringValidator {
    ...
    protected void onValidate(final IValidatable validatable) {
      ...
      if (username != null && !isAvailable(username)) {
        final Map<String, Object> m = new HashMap<String, Object>(1);
