...
public class RenderedLabel extends Image  {
   ...
   * @param shareResource true to add to shared resource pool
   */
  public RenderedLabel(final String id, final IModel model, final boolean shareResource) {
    this(id, model);
    isShared = shareResource;
    init();
  }
  ...
  protected void onBeforeRender() {
    ...
    if (isShared) {
      if (labelHash != curHash) {
        ...
        final SharedResources shared = getApplication().getSharedResources();
  ...
  protected static int getLabelHash(final String text, final Font font, final Color color, final Color backgroundColor, final Integer maxWidth) {
    if (text == null) {
      return 0;
    }
   ...
   */
  protected static void loadSharedResources(final RenderedTextImageResource res, final String text, final Font font, final Color color, final Color backgroundColor, final Integer maxWidth) {
    ...
    final String hash = Integer.toHexString(getLabelHash(text, font, color, backgroundColor, maxWidth));
  ...
  public static class RenderedTextImageResource extends RenderedDynamicImageResource
  {
    ...
    protected boolean render(final Graphics2D graphics)
    {
      ...
      for (final AttributedCharacterIterator attributedIterator : attributedLines) {
        if (maxWidth == null) {
          final TextLayout layout = new TextLayout(attributedIterator, graphics.getFontRenderContext());
          ...
          final LineBreakMeasurer breaker = new LineBreakMeasurer(attributedIterator, graphics.getFontRenderContext());
          TextLayout layout ;
      ...
      final float lineHeight = graphics.getFontMetrics().getHeight(),
    ...
    static List<AttributedCharacterIterator> splitAtNewlines(final AttributedString attr, final String plain) {
   ...
   */
  public static Font fontForResource(final Resource fontRes) {
    try {
