...
public abstract class DataApplication extends DataApplicationBase implements
  ...
  private final HashMap<Object, SessionFactory> hibernateSessionFactories =
      new HashMap<Object, SessionFactory>();
  ...
  /**
   * Initializes a default Hibernate session factory and mounts a page for the
  ...
  /**
   * Called by init to create Hibernate session factory and load a
   * configuration. Passes an empty new AnnotationConfiguration to
   * buildHibernateSessionFactory(key, config) by default. Override if creating
   * a configuration externally.
   * @param key session factory key; the default key is null
   */
  public void buildHibernateSessionFactory(final Object key) {
   ...
   */
  final public void buildHibernateSessionFactory(final Object key,
  ...
  /**
   * Configures the session factory associated with the key. The default
   * implementation calls the configureHibernate(config) method and ignores the
   * key. For applications with multiple session factories, override this method
   * to perform key-specific configuration here instead.
   * @param config configuration to update
   * @param key object, or null for the default factory
   */
  protected void configureHibernate(final AnnotationConfiguration config, final Object key) {
    configureHibernate(config);
  }

  /**
   * For Hibernate settings that should not normally be overriden by client
   * applications. Specifically, this method sets Hibernate for a
   * ManagedSessionContext, the session lookup method used by DataRequestCycle.
   * @param config Hibernate configuration
   */
  protected void configureHibernateEssentials(final AnnotationConfiguration config) {
   ...
   * hbm2ddl auto-updating to create and add columns to tables as required. For
   * deployment it is configured for C3P0 connection pooling.
   ...
   */
  public SessionFactory getHibernateSessionFactory(final Object key) {
    final SessionFactory sf = hibernateSessionFactories.get(key);
    if (sf == null) {
      if (key == null) {
        throw new WicketRuntimeException(
            "The default Hibernate session factory has not been "
   ...
   */
  protected void setHibernateSessionFactory(final Object key, final SessionFactory sf) {
